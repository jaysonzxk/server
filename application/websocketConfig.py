...
def _get_message_center_instance(message_id):
    from apps.admin.models import MessageCenter
...
def _get_message_unread(user_id):
    """获取用户的未读消息数量"""
    from apps.admin.models import MessageCenterTargetUser
...
class DvadminWebSocket(AsyncJsonWebsocketConsumer):
    async def connect(self):
        try:
            ...
            if decoded_result:
                ...
                if unread_count == 0:
                    # 发送连接成功
                    await self.send_json(set_message('admin', 'SYSTEM', '连接成功'))
                else:
                    await self.send_json(
                        set_message('admin', 'SYSTEM', "请查看您的未读消息~",
                                    refresh_unread=True))
    ...
    async def disconnect(self, close_code):
        ...
        await self.channel_layer.group_discard("apps", self.channel_name)
        print("连接关闭")
        try:
