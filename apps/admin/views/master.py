...
class MasterInitSerializer(CustomModelSerializer):
    """
...
class MasterCreateSerializer(CustomModelSerializer):
    """
    师傅新增-序列化器
    """
...
class MasterUpdateSerializer(CustomModelSerializer):
    """
    师傅修改-序列化器
    """
...
class MasterInfoUpdateSerializer(CustomModelSerializer):
    """
    师傅修改-序列化器
    """
    mobile = serializers.CharField(
...
class ExportMasterProfileSerializer(CustomModelSerializer):
    """
    师傅导出 序列化器
    """
...
class MasterViewSet(CustomModelViewSet):
    """
    师傅接口
    list:查询
    create:新增
    ...
    queryset = Users.objects.exclude(is_superuser=1).filter(user_type=2).all()
    serializer_class = MasterSerializer
    create_serializer_class = MasterCreateSerializer
    update_serializer_class = MasterUpdateSerializer
    ...
    export_serializer_class = ExportMasterProfileSerializer
    # 导入
    ...
    def update_user_info(self, request):
        """修改当前用户信息"""
        serializer = MasterInfoUpdateSerializer(request.user, data=request.data, request=request)
        serializer.is_valid(raise_exception=True)
        serializer.save()
